...
class GrayScaler(object):
    def __init__(self, name_topic_in, name_topic_out):
        ...
        self.sub = rospy.Subscriber(name_topic_in, Image, self.image_cb, queue_size=5)
        print("created sub pub")

    def image_cb(self, img_msg):
        # Transform to cv2/numpy image
        print("hello")
        img_in_cv2 = self.cv_bridge.imgmsg_to_cv2(img_msg, desired_encoding='passthrough')
        ...
        if "rgb" in img_msg.encoding:
            rbg_img = cv2.cvtColor(img_in_cv2, cv2.COLOR_RGB2BGR)
        elif "bgr" in img_msg.encoding:
            rgb_img = cv2.cvtColor(img_in_cv2, cv2.COLOR_BGR2RGB)
        # Transform back to Image message
        rbg_img_msg = self.cv_bridge.cv2_to_imgmsg(
            rgb_img, encoding="rgb")
        print("hi")
        self.pub.publish(rgb_img_msg)
